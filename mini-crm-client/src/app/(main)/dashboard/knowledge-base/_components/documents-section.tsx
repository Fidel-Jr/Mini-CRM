...
async function fetchNotes() {
  const response = await fetch("https://localhost:7187/api/Documents");
...
export function DocumentSection() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["documents"], // renamed
    queryFn: fetchNotes,
    select: (data) => ({
      documents: notesSchema.parse(data),
      industries: [],
    }),
  });
  ...
  return (
    <section>
      <Card>
        <CardHeader>
          <CardTitle className="leading-none">Knowledge Base Documents</CardTitle>
          <CardDescription>
            Upload and manage documents that serve as the AI's knowledge base.
          </CardDescription>
          <CardAction>
