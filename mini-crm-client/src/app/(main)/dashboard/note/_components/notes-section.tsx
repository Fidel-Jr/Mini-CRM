...
import { notesColumns } from "./notes-table/columns";
import { notesSchema } from "./notes-table/schema";
...
export function NotesSection() {
  const { data, isLoading, error } = useQuery({
    queryKey: ["notes"], // renamed
    queryFn: fetchNotes,
    select: (data) => ({
      notes: notesSchema.parse(data),
      industries: [],
    }),
  });
  ...
  notes.forEach((c) => {
      if (c.customerName) unique.add(c.customerName);
    });
  ...
  (table.getColumn("customerName")?.getFilterValue() as string) ?? "all";
  ...
  return (
    <section>
      <Card>
        <CardHeader>
          <CardTitle className="leading-none">Recent Notes</CardTitle>
          <CardDescription>
            Track notes across manufacturing, logistics, and other industries.
          </CardDescription>
          <CardAction>
            ...
            <>
              ...
              <div className="flex items-center justify-between gap-4 px-4 pb-1">
                <p className="text-muted-foreground text-sm">
                  Viewing {visibleCustomerCount} out of {filteredCustomerCount.toLocaleString()} notes
                </p>
